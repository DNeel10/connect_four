...
describe Board do
  describe '#drop_marker' do
    ...
    context "when a player selects to drop their marker in a full column once" do
      before do
        selected_column = 4
        board_drop.grid[selected_column] = ['black', 'red', 'black', 'red', 'black', 'red', 'black']
      end

      it 'displays an error message' do
        error_message = 'This column is full, please select another column'
        expect(board_drop).to receive(:puts).with(error_message)
  ...
  describe '#full?' do
    ...
    context "when a column is full of markers" do
      before do
        selected_column = 4
        column_full.grid[selected_column] = ['black', 'red', 'black', 'red', 'black', 'red', 'black']
      end
